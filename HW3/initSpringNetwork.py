...
def initSpringNetwork(N, L, EA, EI, m):
    ...
    mass_per_node = m / (N - 1)
    with open('HW3/springNetworkData/nodes.txt', 'w') as f:
        for i in range(N):
            x_coord = i / (N - 1) * L
    ...
    with open('HW3/springNetworkData/stretch_springs.txt', 'w') as f:
        for i in range(N - 1):
            node1_idx = i
            node2_idx = i + 1
            stiffness = EA
    ...
    with open('HW3/springNetworkData/bending_springs.txt', 'w') as f:
        for i in range(N - 2):
            node1_idx = i
            node2_idx = i + 1
            node3_idx = i + 2
